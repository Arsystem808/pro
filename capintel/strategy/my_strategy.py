...
def _daily(asset_class, ticker, days=520, bars=None):
    """
    Пробуем получить дневные бары через Polygon;
    при 429/ошибке — строим дневку из имеющихся bars (ресемпл).
    """
    # 1) прямой вызов Polygon /v2/aggs 1/day
    try:
...
# ---- генератор сигнала по твоей стратегии ----
def generate_signal_core(ticker, asset_class, horizon, last_price, bars=None) -> Dict[str,Any]:
    p   = _params(horizon); tol = p['tol']
    # дневку берём с фолбэком и передаём bars
    daily = _daily(asset_class, ticker, 520, bars=bars)
    # если вдруг пусто — хотя бы не падаем
    if daily.empty and bars is not None and not bars.empty:
        daily = bars.copy()
        if "dt" in daily.columns:
            daily = daily.set_index(pd.to_datetime(daily["dt"], utc=True))
        elif "t" in daily.columns and not isinstance(daily.index, pd.DatetimeIndex):
            daily = daily.set_index(pd.to_datetime(daily["t"], unit="s", utc=True))
        daily = daily[["o","h","l","c"]].astype(float)

    H,L,C = _prev_hlc(daily, p['period'])
    piv   = _fibo(H,L,C)
    ...
    hist      = _macd_hist(b['c']); rsi = _rsi(b['c'],14); atr = _atr(b,14)

    px = float(last_price)
    last_atr = float(atr.iloc[-1]) if len(atr) else px*0.006
    ...
    if overheat:
        action='SHORT'
        if nR3:
            tp1, tp2, stop = piv['R2'], piv['P'], piv['R3']*(1+tol)
        else:
            tp1, tp2, stop = (piv['P']+piv['S1'])/2.0, piv['S1'], piv['R2']*(1+tol)
    elif oversold:
        action='BUY'
        if nS3:
    ...
    alt  = dict(if_condition='подтверждение на уровне (rejection/смена цвета HA)',
                action=action, entry=entry, take_profit=[tp1,tp2], stop=stop)

    return dict(
        action=action, entry=entry, take_profit=[float(tp1),float(tp2)], stop=float(stop),
