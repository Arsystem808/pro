...
# Устойчивый импорт storyteller'а
try:
    ...
    def trader_tone_narrative_ru(sig: Dict[str, Any], bars: pd.DataFrame):
        # Фоллбэк: просто вернуть существующие тексты
...
def _fetch_bars(asset_class: str, ticker: str, horizon: str) -> pd.DataFrame:
    return intraday_bars(asset_class, ticker, interval="5m", limit=520) if horizon=="intraday" else \
           daily_bars(asset_class, ticker, limit=520)
