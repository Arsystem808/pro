...
def _headers():
    if not POLYGON_API_KEY:
        raise PolygonError("POLYGON_API_KEY не задан. Укажи ключ в окружении/Secrets.")
...
def last_trade_equity(ticker: str) -> float:
    ...
    with httpx.Client(timeout=10) as c:
        ...
        if r.status_code == 200:
            data = r.json()
            price = (data.get("results") or {}).get("price")
...
def last_trade_crypto(pair: str) -> float:
    ...
    with httpx.Client(timeout=10) as c:
        ...
        d2 = c.get(url2, headers=_headers()).json()
        res = (d2 or {}).get("results") or []
        if res: return float(res[0].get("c"))
...
def _timespan_for_horizon(h: str):
    return {"intraday":("minute",5), "swing":("hour",1), "position":("day",1)}.get(h, ("minute",5))

def get_agg_bars(asset_class: str, ticker: str, horizon: str, limit: int = 400):
    """
    Универсальный загрузчик агрегатов. При 429/ошибке — возвращает пустой DataFrame,
    чтобы стратегия сделала фолбэк и не роняла приложение.
    """
    import pandas as pd
    ts, mult = _timespan_for_horizon(horizon)
    if asset_class == 'crypto':
        b,q = _norm_crypto_pair(ticker); tkr = f"X:{b}{q}"
    else:
        tkr = ticker.upper()
